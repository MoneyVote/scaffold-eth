...
describe("Voting contract", function () {
    ...
    describe("Voting Transactions", function () {
        it("Should allow a vote for a candidate from two different voters", async function () {
            ...
            await hardhatVoting.connect(owner).voteForCandidate(ethers.utils.formatBytes32String('Sean Connery'));
        ...
        it("Should not allow a 2nd vote from the same voter", async function () {
            await hardhatVoting.connect(owner).voteForCandidate(ethers.utils.formatBytes32String('Sean Connery'));
            await expect(
                hardhatVoting.connect(owner).voteForCandidate(ethers.utils.formatBytes32String('Sean Connery'))
            ).to.be.reverted;
        });
    });
